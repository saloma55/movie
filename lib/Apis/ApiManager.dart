...
import 'package:movies/Models/SimilarMovies.dart';
...
class ApiManager {
  ...
  static Future<CategoryFilter> categoryfilter(num categoryid) async {
    Uri url = Uri.https(
      baseUrl,
      "/3/discover/movie",
      {
  ...
  static Future<SimilarMovies> getSimilarMovies(num movie_id) async {
    Uri url = Uri.https(
      baseUrl,
    ...
    var response = await http.get(url, headers: headers);
    Map<String, dynamic> json = jsonDecode(response.body);
    SimilarMovies similars = SimilarMovies.fromJson(json);
    return similars;
  }
}
